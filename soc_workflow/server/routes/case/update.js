...
export default function (server, options) {
    const update = (req, reply) => {
        ...
        if (typeof req.query.dateFrom != "undefined") {
            dateFrom = moment((req.query.dateFrom * 1000)).startOf('day');
        }
        let dateTo = moment();
        if (typeof req.query.dateTo != "undefined") {
            dateTo = moment((req.query.dateTo * 1000)).endOf('day');
        }
        ...
        Promise.all([
            ...
            ecsGetAggregatedByField(server, req, 'case_ecs-*', 'event.severity', dateFrom, dateTo, presetColorGlobalPriority), // donutByPriority
